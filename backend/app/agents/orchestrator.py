    ...
    def create_dynamic_crew(self, config: MaterialConfig) -> Crew:
        ...
        skribent = Agent(
            ...
            backstory=(
                ...
                "TYPST-REGLER (KRITISK!):\n"
                "- Overskrifter: #heading(level: 1)[Tekst]\n"
                "- Matematikk inline: $x^2 + 2x + 1$\n"
                "- Matematikk utstilt: $ x^2 + 2x + 1 $\n"
                "- Brøk: $frac(a, b)$ IKKE \\frac\n"
                "- Potens: $x^2$ eller $x^(2n)$\n"
                "- Grenser: $lim_(x -> 0)$ IKKE arrow\n"
                "- Cases: $cases(a \"for\" x>0, b \"for\" x<0)$ UTEN &\n"
                "- Enheter: $5 \"m\"$ med mellomrom før enheten\n"
                "- Multiplikasjon: $a cdot b$ eller $a times b$\n"
                "- Integral: $integral f(x) dif x$ IKKE d x\n"
                "- Piler: -> og <- IKKE arrow\n"
                "- Sideskift: #pagebreak()\n"
                "- IKKE bruk LaTeX-kommandoer\n"
                "- IKKE definer egne #let variabler"
            ),
            llm=self.llm,
            allow_delegation=False,
            max_iter=2
